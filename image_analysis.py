...
from scipy import spatial, signal, ndimage, misc, stats
...
def interpMax(arr, heights=(0.,1.,2.)):
    ...
    i = ((max_heights < max(heights))*(max_heights > min(heights))) == False
...
class Stack():
    ...
    #     while not self.focuses.empty():
    #         printProgress(ind, whole)
    #         im = self.focuses.get()
    #         im.focus()
    #         ind += 1
    #     printProgress(1, 1)
...
class dialStack():
    ...
    def getMotion(self):
        ...
        #individual ticks throughout the video. Jamie suggests that this'll
    ...
    def angleRotated(self, prev_kp, curr_kp, center=None):
        ...
        curr_pos = curr_pos[distance != inf]
        index = index[distance != inf]
        distance = distance[distance != inf]
...
class Video(Stack):
    ...
    def __init__(self, filename, fps=30, f_type=".jpg"):
        ...
        if (os.path.isfile(self.filename) and
            ...
            s = "ffprobe -v error -select_streams v:0 -show_entries\
...
class colorSelector():
    ...
    def selectColor(self, dilate=5):
        ...
        # hsv = filters.gaussian_filter(hsv, 3)
        # self.gauss = hsv
        low_hue = self.colors[:, 0].min()
        hi_hue = self.colors[:, 0].max()
        if low_hue < 0:
...
#     st = stack()
#     st.focusStack()
#     misc.imsave('focus_stack.jpg', st.stack)   

# z = np.zeros(fr.shape)
# d = 25
# for [x, y] in ps2:
#     z[y-d:y+d, x-d:x+d] = True
#     z[-(y+d):-(y-d), x-d:x+d] = True
#     z[-(y+d):-(y-d), -(x+d):-(x-d)] = True
#     z[y-d:y+d, -(x+d):-(x-d)] = True
